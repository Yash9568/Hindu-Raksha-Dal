...
export async function POST(req: Request) {
  try {
    ...
    const existing = await prisma.user.findFirst({ where: { OR: [{ email: emailNorm }, ...(phoneNorm ? [{ phone: phoneNorm }] : [])] } });
    if (existing) {
    ...
    const user = await prisma.user.create({
      data: { name, email: emailNorm, phone: phoneNorm ?? undefined, photoUrl, passwordHash },
